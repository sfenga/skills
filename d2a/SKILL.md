...
Mini 是完整版本的架构学习模型，不是另起炉灶的简化实现。它可以减少功能、数据规模、
外部集成和工程化细节，但应尽可能保持完整版本的组件职责、调用与依赖方向、状态所有权、
边界和错误语义。不得为了少写代码而把多个完整版本组件的职责合并到一个 Mini 组件，或把
状态和边界责任转移给不同层。任何无法保持的架构偏差都必须单独记录理由并取得用户确认。
...
- Mini 从范围阶段开始维护 `mini/architecture-alignment.json`，逐项映射完整版本与 Mini 的
  组件职责，并证明调用方向、状态所有权和边界语义一致。构建与测试阶段必须补上真实的
  `.d2a/mini/source` 代码锚点；只缩减实现细节，不得借缩减重新分配架构职责。
